...
const EditRepo = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.put(`${REACT_APP_BASE_URL}/repo/${params.repoId}`, values, { withCredentials: true });
        if (response) {
  ...
  const getLatestCommit = async () => {
    try {
      const response = await axios.get(`${REACT_APP_BASE_URL}/repo/${params.repoId}`, { withCredentials: true });
      if (response) {
